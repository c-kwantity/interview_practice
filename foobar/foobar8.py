...
def solution(num_buns, num_required):
    bunniesLeft = num_buns - num_required + 1
    # we find the number of ways we need to spread the keys amongst the bunnies left uniquely
    # n! / (n-k)!k! to get the number of locks needed
    locksNeeded = factorial(num_buns) / (factorial(num_buns - bunniesLeft) * factorial(bunniesLeft))
    keysAllocation = [[] for i in xrange(num_buns)]
    bunnies = [i for i in xrange(num_buns)]
    for lock,keysDistribution in enumerate(combinations(bunnies, bunniesLeft)):
        for bunny in keysDistribution:
            keysAllocation[bunny].append(lock)
    return keysAllocation
